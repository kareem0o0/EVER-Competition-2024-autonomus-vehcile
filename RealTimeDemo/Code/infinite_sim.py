...
def init_node():
    ...
    rospy.Subscriber('/odom', Odometry, callvack)

    cmd_pub = rospy.Publisher("/cmd_vel", Float64, queue_size=1) 

    brake_pub = rospy.Publisher("/brakes", Float64, queue_size= 1)
...
def callvack(odom):
    ...
    for i in range (0, num_path_values):
        ...
        if int(distance) == 4 and angle_diff>=-5 and  angle_diff<=60: #tunable
            point =  path[i]
...
def calculate_curv(point):
   ...
   # Calculate relative position 
    ...
    cmd_pub.publish(.2) #Tunable but not that important at first
'''
