...
x_values = csv_reading(file_path, column_x)          #np.linspace(-3.832, 3.832, 15)            
y_values = csv_reading(file_path, column_y)          #np.linspace(-2.375, 2.375, 15)

right_to_left = list(zip(x_values, y_values))

y_values = csv_reading(file_path, column_y)[::-1]    #np.linspace(2.375, -2.375, 15)
...
def calculate_curv(point):
   # Calculate relative position 
    ...
    # depth camera
    defaul_steering = steering_angle_deg
    ...
    if (depth <= 2000 and depth > 0):   #tuning
        ...
        steering_angle_deg = defaul_steering
